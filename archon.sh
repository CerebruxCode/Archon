...
function installer() {
	...
	sleep 2
	echo
	if pacman -S --noconfirm "${@:2}"; then # Με ${*2} διαβάζει τα input
		echo
...
function chroot_stage {
	...
	echo
	#########################################################
	until passwd; do # Μέχρι να είναι επιτυχής
	...
	#########################################################
	until passwd "$onomaxristi"; do # Μέχρι να είναι επιτυχής
		# η εντολή
	...
	if YN_Q "Θέλετε να δημιουργήσετε swapfile (y/n); " "μη έγκυρος χαρακτήρας"; then
		...
		echo
		while :; do # Δικλείδα ασφαλείας αν ο χρήστης προσθέσει μεγάλο νούμερο.
...
function diskchooser() {
	...
	while true; do
		...
		if [ "$input" -gt 0 ] && [ "$input" -le $num ]; then #έλεγχος αν το input είναι μέσα στο εύρος της λίστας
			if [[ $1 = "grub" ]]; then                          # αν προστεθεί το όρισμα grub τότε η μεταβλητή που θα αποθηκευτεί
				# θα είναι η grubvar
