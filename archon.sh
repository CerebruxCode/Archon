...
# Ανάλογα τις γλώσσες που θα έχουμε στην μετάφραση στην διαδικασία εγκατάστασησ θα έχουμε και αντίστοιχα txt
# αρχεία 
function language() {
    clear
    menu_languages
	while true; do
		...
		case $epilogi in
			[1] )                   # Εδώ ανάλογα με την επιλογή
				input="lang_gr.txt"
                gr_color="${IYellow}"
                en_color="${IWhite}"  # της γλώσσας δίνει σαν όνομα
				break
				;;
...
function chroot_stage {
	...
	sleep 2
	echo
	read -rp "${str_lang1[25]} " hostvar
	echo "$hostvar" > /etc/hostname
	echo
	sleep 2
    echo -e "$slim"
	echo -e "${IGreen}${str_lang1[26]}${NC}"       
	echo
	echo -e "${str_lang1[27]}"
	echo -e "${str_lang1[28]}"
	echo -e "${str_lang1[29]}"
	...
	if [ "$ethernet" = "" ]; then					# Έλεγχος αν υπάρχει κάρτα ethernet
		echo -e "${IYellow}${str_lang1[30]}${NC}"				# και αν υπάρχει γίνεται εγκατάσταση
	else 								# και ενεργοποίηση
		   systemctl enable dhcpcd@"$ethernet".service
		echo -e "${IGreen}${str_lang1[31]} $ethernet ${str_lang1[32]}${NC}";
	fi
	echo
	wifi=$(ip link | grep ": "| grep -oE "(w\\w+)")			# Αναζήτηση κάρτας wifi
	if [ "$wifi" = "" ]; then					# Έλεγχος αν υπάρχει κάρτα wifi
		echo -e "${IYellow}${str_lang1[33]}${NC}"		# και αν υπάρχει γίνεται εγκατάσταση
	else 								# και ενεργοποίηση
		pacman -S --noconfirm iw wpa_supplicant dialog wpa_actiond
		systemctl enable netctl-auto@"$wifi".service
		echo -e "${IGreen}${str_lang1[34]} $wifi ${str_lang1[35]}${NC}"
	fi
	sleep 2
	echo
	echo -e "$slim"
	echo -e "${IGreen}${str_lang1[36]}${NC}"
	echo
	echo -e "${str_lang1[37]}"
	echo -e "${str_lang1[38]}"
	echo -e "$slim"
	...
	echo -e "${IYellow}${str_lang1[39]}${NC}"	# τυπώνεται αυτό το μήνυμα
	echo							#
	done							#
	#########################################################
	sleep 2
	...
	echo -e "$slim"
	sleep 2
	if YN_Q "${str_lang1[44]} "; then
		sudo pacman -S --noconfirm linux-lts
	fi
	echo
	echo
	echo -e "$slim"
	echo -e "${IGreen}${str_lang1[45]}${NC}"
	echo
	echo -e "${str_lang1[46]}"
	echo -e "${str_lang1[47]}"
    echo -e "$slim"
	...
	if [ $filesize -ne 0 ]; then
		num=0
  		while IFS='' read -r line || [[ -n "$line" ]]; do
		    ...
		    mount $line /run/media/disk$num | echo -e "${IYellow}${str_lang1[48]}"$num"${str_lang1[49]}${NC}"
		    sleep 1
      
		  done < "disks.txt"

		else
		  echo -e "${IYellow}${str_lang1[50]}${NC}"
	fi
	sleep 5
	rm disks.txt
	...
	echo ""
	echo -e "$slim"
	echo -e "${IGreen}${str_lang1[67]}${NC}"
	echo
	echo -e "${str_lang1[68]}"
	echo -e "${str_lang1[69]}"
	echo
	echo -e "         ${IGreen}${str_lang1[70]}${NC}"
	echo -e "${str_lang1[71]}"
	echo -e "${str_lang1[72]}"
	echo -e "${str_lang1[73]}"
	echo -e "${str_lang1[74]}"
	...
	sleep 2
	############# Installing Desktop ###########
	if YN_Q "${str_lang1[77]} " "${str_lang1[78]}" ; then
		echo ""
		echo -e "${IYellow}${str_lang1[79]}${NC}"
		check_if_in_VM
    	initialize_desktop_selection
	else
		echo -e "${IYellow}${str_lang1[80]}${NC}"
		exit 0
	fi
}
